...
async def sudo_panel_callback(event):
    ...
    if action == "main":
        ...
        # تعديل لحساب المجموعات فقط (التي تبدأ بـ '-')
...
"""
    ...
    # --- (جديد) قسم صيانة قاعدة البيانات ---
    elif action == "db_maint":
        await event.edit("**🛠️ | جاري فحص قاعدة البيانات بحثاً عن المجموعات غير النشطة... قد يستغرق هذا وقتاً طويلاً.**")
        ...
        for chat_id_str in all_chats:
            try:
                await client.get_entity(int(chat_id_str))
                await asyncio.sleep(1) # فاصل زمني لتجنب الضغط على السيرفر
            except (ValueError, UserNotParticipantError, TypeError):
                inactive_chats.append(chat_id_str)
            except Exception as e:
                # هذا الخطأ يعني أن البوت غير موجود في المجموعة
                if "You are not a member of this supergroup" in str(e):
                    inactive_chats.append(chat_id_str)
                else:
                    print(f"DB Maint check error for {chat_id_str}: {e}")

        if not inactive_chats:
            return await event.edit("**✅ | قاعدة البيانات نظيفة! لم يتم العثور على مجموعات غير نشطة.**", buttons=[Button.inline("🔙 رجوع", data="sudo_panel:main")])

        for chat_id_str in inactive_chats:
            del db[chat_id_str]
        save_db(db)
        
        await event.edit(f"**🗑️ | اكتملت الصيانة!**\n\n**تم تنظيف وحذف بيانات `{len(inactive_chats)}` مجموعة غير نشطة من قاعدة البيانات.**", buttons=[Button.inline("🔙 رجوع", data="sudo_panel:main")])

    # --- (جديد) قسم فحص البيانات ---
    elif action == "inspect":
        inspect_text = "**🔍 | قسم فحص البيانات**\n\n**اختر نوع البيانات التي تريد فحصها:**"
        inspect_buttons = [
        ...
        try:
            async with client.conversation(event.sender_id, timeout=300) as conv:
                await conv.send_message("**🏙️ | أرسل الآن ID المجموعة التي تريد فحص بياناتها...**\n**(لا تنسَ إشارة السالب `-` للمجموعات)**")
                response = await conv.get_response()
                ...
                # بناء التقرير
                report = f"**📄 | تقرير بيانات المجموعة `{chat_id_str}`**\n\n"
                report += f"**- عدد الأعضاء المسجلين:** {len(chat_data.get('users', {}))}\n"
                report += f"**- عدد الأدمنية:** {len(chat_data.get('bot_admins', []))}\n"
                report += f"**- الترحيب:** {'موجود' if chat_data.get('welcome_message') else 'غير محدد'}\n"
                report += f"**- القوانين:** {'موجودة' if chat_data.get('rules') else 'غير محددة'}\n"
        ...
        try:
            async with client.conversation(event.sender_id, timeout=300) as conv:
                await conv.send_message("**👤 | أرسل الآن ID المستخدم الذي تريد فحصه...**")
                response = await conv.get_response()
                user_id_str = response.text.strip()
                
                report = f"**📄 | تقرير بيانات المستخدم `{user_id_str}`**\n\n"
                is_gbanned = user_id_str in (str(uid) for uid in db.get("globally_banned", []))
                report += f"**- حالة الحظر العام:** {'محظور 🚫' if is_gbanned else 'غير محظور ✅'}\n\n"
                
                groups_found_in = []
                total_msgs = 0
                for chat_id, chat_data in db.items():
                    if isinstance(chat_data, dict) and "users" in chat_data:
                        if user_id_str in chat_data["users"]:
                            groups_found_in.append(chat_id)
                            total_msgs += chat_data["users"][user_id_str].get("msg_count", 0)
                
                report += f"**- مجموع الرسائل في كل المجموعات:** {total_msgs}\n"
                report += f"**- موجود في:** `{len(groups_found_in)}` **مجموعة**\n"
