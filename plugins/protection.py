...
import config
from .utils import has_bot_permission, get_user_rank, Ranks, get_or_create_user, get_or_create_chat, set_chat_setting
...
async def lock_unlock_handler(event):
    try:
        ...
        action, target = event.pattern_match.group(1), event.pattern_match.group(2).strip()
...
async def unmute_handler(event):
    try:
        ...
        logger.error(f"Error in unmute_handler: {e}", exc_info=True)
        await event.reply(f"**ماكدرت افك الكتم، اكو مشكلة 😢:**\n`{e}`")

@client.on(events.NewMessage(pattern=r"^[!/]ضع عدد التحذيرات (\d+)$"))
async def set_warns_limit_handler(event):
    try:
        ...
        logger.error(f"Error in set_warns_limit_handler: {e}", exc_info=True)
        await event.reply("**صارت مشكلة وماكدرت اضبط العدد 😢**")

@client.on(events.NewMessage(pattern=r"^[!/]ضع وقت الكتم (\d+)$"))
async def set_mute_duration_handler(event):
    try:
        actor_rank = await get_user_rank(event.sender_id, event.chat_id)
        if actor_rank < Ranks.CREATOR:
            return await event.reply("**هاي الشغلات بس للمنشئين والمالك 👑**")
        ...
        logger.error(f"Error in set_mute_duration_handler: {e}", exc_info=True)
        await event.reply("**صارت مشكلة وماكدرت اضبط الوقت 😢**")
        
@client.on(events.NewMessage(pattern=r"^[!/](تشغيل|تعطيل) صورة ايدي$"))
async def toggle_id_photo_handler(event):
    try:
        ...
        logger.error(f"Error in toggle_id_photo_handler: {e}", exc_info=True)
...
@client.on(events.NewMessage(pattern=r"^[!/]الغاء الحظر$"))
...
async def clear_warns_handler(event):
    if not await has_bot_permission(event): return
    reply = await event.get_reply_message()
    if not reply: return await event.reply("**على منو بالضبط؟ رد على رسالته علمود اعرفه 🧐**")
...
async def remove_filter_handler(event):
    if not await has_bot_permission(event): return
    word_to_remove = event.pattern_match.group(1).strip().lower()
...
@client.on(events.NewMessage(pattern=r"^[!/]الكلمات الممنوعة$"))
