...
async def handle_interactive_callback(event):
        ...
        if sub_action == "read":
            async with AsyncDBSession() as session:
                ...
                if user_id == whisper_data.to_id:
                    ...
                    await event.answer(whisper_text, alert=True)
    ...
    if action == "rps":
        ...
        async with AsyncDBSession() as session:
            ...
            if not game: return await event.answer("🚫 | **هذا التحدي قديم أو انتهى.**", alert=True)
            p1_id, p2_id = game.player1_id, game.player2_id
            p1_id_from_data = int(data_parts[2])
            p2_id_from_data = int(data_parts[3])
            if user_id not in [p1_id_from_data, p2_id_from_data]: return await event.answer("🤨 | **هذا التحدي مو إلك!**", alert=True)
            player_choice = data_parts[1]
            if user_id == p1_id:
                if game.player1_choice: return await event.answer("✅ | **انتظر خصمك، انت اخترت خلاص.**", alert=True)
                game.player1_choice = player_choice
            else:
                if game.player2_choice: return await event.answer("✅ | **انتظر خصمك، انت اخترت خلاص.**", alert=True)
            ...
            if game.player1_choice and game.player2_choice:
                ...
                if p1_c == p2_c: winner_text = "**تعادل! ماكو فايز.**"
    ...
    if action == "xo":
        ...
        if not game: return await event.answer("🚫 | **هذه اللعبة قديمة أو انتهت.**", alert=True)
        if data_parts[1] == "done": return await event.answer("اللعبة منتهية!", alert=True)
        if user_id != game['turn']: return await event.answer("😒 | **مو سرآك هسه!**", alert=True)
        pos = int(data_parts[1])
        if game['board'][pos] != '-': return await event.answer("❌ | **هذا المكان محجوز! اختر مكان فارغ.**", alert=True)
    ...
    if action == "tasbeeh":
        ...
        if sub_action == "click":
            ...
            new_buttons = [[Button.inline(f"{zikr} [{current}]", data=new_data), Button.inline("🔄 إعادة التصفير", data=reset_data)]]
            try:
            ...
            reset_data = f"tasbeeh:reset:{zikr}:{goal}:0"
            new_buttons = [[Button.inline(f"{zikr} [0]", data=new_data), Button.inline("🔄 إعادة التصفير", data=reset_data)]]
            await event.edit(buttons=new_buttons)
    ...
    if action == "mahbis":
        ...
        if not game: return await event.answer("هاي اللعبة خلصانة.", alert=True)
        player_id = game["player_id"]
        if user_id != player_id: return await event.answer("هاي اللعبة مو الك! اذا تريد تلعب، اكتب `محيبس`", alert=True)
    ...
    if action == "proposal":
        ...
        msg_id = event.message_id
        if user_id != proposed_id: return await event.answer("**هذا الطلب موجه لشخص آخر.**", alert=True)
        proposal_data = PROPOSALS.get(msg_id)
        if not proposal_data: return await event.answer("**هذا الطلب قديم أو انتهت صلاحيته.**", alert=True)
