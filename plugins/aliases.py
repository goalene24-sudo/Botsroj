...
async def add_alias_handler(event):
    ...
    try:
        # --- (تم التعديل) بدء المحادثة في المجموعة بدلاً من الخاص ---
...
**""")
            ...
            # سيتم انتظار الرد من نفس الشخص الذي بدأ الأمر
            original_command_msg = await conv.get_response(from_users=event.sender_id)
...
**""")
            ...
            alias_command_msg = await conv.get_response(from_users=event.sender_id)
            ...
            if alias_command in db[chat_id_str]["command_aliases"]:
                await original_command_msg.reply(f"**⚠️ عذراً، الاختصار `{alias_command}` مستخدم بالفعل. حاول مرة أخرى باختصار مختلف.**")
                return
